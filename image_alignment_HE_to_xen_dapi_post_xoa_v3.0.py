...
parser.add_option("-d", "--dapi",
                  action="store", type="string", dest="dapi_path", default = "",
                  help="path to the input dapi image from xenium platform (use the morphology_focus_0000.ome.tif file)")
...
parser.add_option("--multimodal_path",
                  action="store", type="string", dest="multimodal_path", default="",
                  help="path to the folder containing the multimodal images from xenium platform (example: '/Xenium_run_folders/output-XETG00122__0037704__HT910P1-S1H1Fp1U1__20240626__183338/morphology_focus/'). This is the folder that contains the 4 files: morphology_focus_0000.ome.tif, morphology_focus_0001.ome.tif, morphology_focus_0002.ome.tif, and morphology_focus_0003.ome.tif. Do not rename the files.")
...
if options.dapi_path != "" :
    print("Path to dapi image provided. Proceeding with alignment to just the dapi image. To align to the full multimodal image provide just the path to the multimodal image folder with the `--multimodal_path` flag. For more information use the `--help` flag.")
    dapi_image = np.asarray(tifffile.imread(options.dapi_path, is_ome=False, level=options.downscaling))
    ...
    print("gray scaled xenium multimodal image stats")
    print(np.max(dapi_image_inverted_small))
    print(np.min(dapi_image_inverted_small))
    print(dapi_image_inverted_small.shape)
elif options.multimodal_path != "":
    print("Path to the multimodal image folder provided. Proceeding with alignment to the multimodal image. To align H&E to the dapi image provide just the path to the dapi image using the `-d` flag. for more information use the `--help` flag.")
    img_0000_path = options.multimodal_path + "morphology_focus_0000.ome.tif"
    img_0001_path = options.multimodal_path + "morphology_focus_0001.ome.tif"
    img_0002_path = options.multimodal_path + "morphology_focus_0002.ome.tif"
    img_0003_path = options.multimodal_path + "morphology_focus_0003.ome.tif"
    img_0000 = np.asarray(tifffile.imread(img_0000_path, is_ome=False, level=options.downscaling))
    img_0001 = np.asarray(tifffile.imread(img_0001_path, is_ome=False, level=options.downscaling))
    img_0002 = np.asarray(tifffile.imread(img_0002_path, is_ome=False, level=options.downscaling))
    img_0003 = np.asarray(tifffile.imread(img_0003_path, is_ome=False, level=options.downscaling))
    xen_color = np.asarray([img_0000, img_0001, img_0002, img_0003])
    xen_grey = np.mean(xen_color, axis=0)
    dapi_image_scale_0_1 = (xen_grey - np.min(xen_grey))/(np.max(xen_grey)-np.min(xen_grey))
    dapi_image_scale_0_255 = dapi_image_scale_0_1*255
    #dapi_image_scale_0_255 = np.floor(dapi_image_scale_0_255)
    dapi_image_scale_0_255 = dapi_image_scale_0_255.astype(np.uint8)
    dapi_image_inverted_small = 255 - dapi_image_scale_0_255
    print("gray scaled xenium multimodal image stats")
    print(np.max(dapi_image_inverted_small))
    print(np.min(dapi_image_inverted_small))
    print(dapi_image_inverted_small.shape)
    print(dapi_image_inverted_small)
else:
    print("Neither the path to the dapi image or the multimodal stain folder was provided. Alignment will not proceed. Specify either the dapi image path with the `-d` flag or the multimodal image folder with the `--multimodal_path` flag. Do not provide both. For more information use the `--help` flag.")
    sys.exit()

skimage.io.imsave("dapi_gray_scale_target.tif", dapi_image_inverted_small)
if options.flip:
...
if options.dapi_path != "" :
    print("Path to dapi image provided. Proceeding with alignment to just the dapi image. To align to the full multimodal image provide just the path to the multimodal image folder with the `--multimodal_path` flag. For more information use the `--help` flag.")
    dapi_image = np.asarray(tifffile.imread(options.dapi_path, is_ome=False, level=0))
    #dapi_image = cv2.resize(dapi_image, (dapi_image.shape[1]//he_resize_scale, dapi_image.shape[0]//he_resize_scale))
    dapi_image_scale_0_1 = (dapi_image - np.min(dapi_image))/(np.max(dapi_image)-np.min(dapi_image))
    dapi_image_scale_0_255 = dapi_image_scale_0_1*255
    dapi_image_scale_0_255 = dapi_image_scale_0_255.astype(np.uint8)
    dapi_image_inverted = 255 - dapi_image_scale_0_255
    print("gray scaled xenium multimodal image stats")
    print(np.max(dapi_image_inverted))
    print(np.min(dapi_image_inverted))
    print(dapi_image_inverted.shape)
    print(dapi_image_inverted)
elif options.multimodal_path != "":
    print("Path to the multimodal image folder provided. Proceeding with alignment to the multimodal image. To align H&E to the dapi image provide just the path to the dapi image using the `-d` flag. for more information use the `--help` flag.")
    img_0000_path = options.multimodal_path + "morphology_focus_0000.ome.tif"
    img_0001_path = options.multimodal_path + "morphology_focus_0001.ome.tif"
    img_0002_path = options.multimodal_path + "morphology_focus_0002.ome.tif"
    img_0003_path = options.multimodal_path + "morphology_focus_0003.ome.tif"
    img_0000 = np.asarray(tifffile.imread(img_0000_path, is_ome=False, level=0))
    img_0001 = np.asarray(tifffile.imread(img_0001_path, is_ome=False, level=0))
    img_0002 = np.asarray(tifffile.imread(img_0002_path, is_ome=False, level=0))
    img_0003 = np.asarray(tifffile.imread(img_0003_path, is_ome=False, level=0))
    xen_color = np.asarray([img_0000, img_0001, img_0002, img_0003])
    xen_grey = np.mean(xen_color, axis=0)
    dapi_image_scale_0_1 = (xen_grey - np.min(xen_grey))/(np.max(xen_grey)-np.min(xen_grey))
    dapi_image_scale_0_255 = dapi_image_scale_0_1*255
...
print("scaled_M")
print(scaled_M)
np.savetxt("scaled_M.csv", scaled_M, delimiter=",")
pts = np.float32([ [0,0],[0,h-1],[w-1,h-1],[w-1,0] ]).reshape(-1,1,2)
...
print("perspectiveM_large")
print(perspectiveM_large)
np.savetxt("perspectiveM_large.csv", perspectiveM_large, delimiter=",") 
